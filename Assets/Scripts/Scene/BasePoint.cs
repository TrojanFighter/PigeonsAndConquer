...
public class BasePoint : MonoBehaviour {
	...
	Fraction fraction;
	public int FractionHP, FractionMP, FractionHumanResource;
	...
	void FixedUpdate(){
		...
		if (LookAtGodOne) {
			...
			GameManager.BasePoints [GlobalDefine.Fraction.One].FractionGod.transform.position += new Vector3 (0, 30f*Time.fixedDeltaTime, 0);
		} else if (LookAtGodTwo) {
			Camera.main.transform.LookAt (GameManager.BasePoints [GlobalDefine.Fraction.Two].FractionGod.transform);
			GameManager.BasePoints [GlobalDefine.Fraction.Two].FractionGod.transform.position += new Vector3 (0, 30f*Time.fixedDeltaTime, 0);
		}
	}
	...
	public void ChangeBaseValue(int HP,int MP,int HumanResource){
		FractionHP += HP;
		FractionMP += MP;
		FractionHumanResource += HumanResource;
